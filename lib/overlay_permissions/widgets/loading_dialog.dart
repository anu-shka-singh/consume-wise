...
class LoadingBar extends StatefulWidget {
  ...
  static showLoadingDialog(
      ...
      required Function statusFunction,
...
class _LoadingBarState extends State<LoadingBar> {
  ...
  void initState() {
    ...
    Timer.periodic(Duration(seconds: widget.timeToCheck), (timer) {
      ...
      int newStatus =
          timePassed >= widget.timeoutInSeconds ? 3 : widget.statusFunction();
  ...
  Widget build(BuildContext context) {
    ...
    if (status == 1) {
      alertText = Center(
          child: Text(
        _successMessagePresent()
            ? widget.messagesMap!["successMessage"]!
            : "Success!",
        style: TextStyle(fontSize: screenWidth * 0.06),
      ));
      alertContent = const Icon(
    ...
    if (status == 2) {
      alertText = Center(
          child: Text(
              _failureMessagePresent()
                  ? widget.messagesMap!["failureMessage"]!
                  : "Failed!",
              style: TextStyle(fontSize: screenWidth * 0.06)));
      alertContent = const Icon(
        Icons.close_rounded,
  ...
  bool _successMessagePresent() {
    if (widget.messagesMap != null &&
        widget.messagesMap!.containsKey("successMessage")) {
      return true;
    } else {
      return false;
    }
  ...
  bool _failureMessagePresent() {
    if (widget.messagesMap != null &&
        widget.messagesMap!.containsKey("failureMessage")) {
      return true;
    } else {
      return false;
    }
